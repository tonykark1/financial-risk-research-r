extract_feature_importance <- function(fitted) {
  ...
  if (fitted$model %in% c("Ridge", "Elastic_Net")) {
    ...
    return(data.table::data.table(feature = predictors,
      importance = abs(coefficient), signed_effect = coefficient))
  }
  if (fitted$model == "Random_Forest") {
    importance <- fitted$fit$variable.importance
    return(data.table::data.table(feature = names(importance),
      importance = as.numeric(importance), signed_effect = NA_real_))
  }
  if (fitted$model == "XGBoost") {
    ...
    return(data.table::data.table(feature = importance[[feature_column]],
      importance = importance$Gain, signed_effect = NA_real_))
  }
  coefficient <- fitted$fit$coefficients[-1L]
  data.table::data.table(feature = predictors, importance = abs(coefficient),
...
run_macro_feature_stability <- function(macro_data, config, horizon = 22L) {
  ...
  write_parquet_duckdb(result,
    project_path("data/gold/macro_vol/feature_stability.parquet",
